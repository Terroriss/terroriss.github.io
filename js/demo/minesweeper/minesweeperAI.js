...
const isSameSet = (set1, set2) => {
    if (!(set1 instanceof ArraySet) || !(set2 instanceof ArraySet)) return false
    if (set1.size != set2.size) return false
    const iterator1 = set1.values();
    for (i = 0; i < set1.size; i++) {
        let item = iterator1.next().value
        if (!set2.has(item)) return false
    }
    return true
}

const setInArr = (set, arr) => {
...
const minusSet = (s1, s2) => {
   let newSet = new ArraySet();
...
function union(setA, setB) {
    let _union = setA
    const iterator = setB.values();
    for (i = 0, len = setB.size; i < len; i++) {
...
isSubSet = function(set, otherSet) {
...
class Sentence {
    ...
    mark_mine(cell) {
        if (this.cells.delete(cell)) this.count -= 1
    }
...
class MinesweeperAI {
    constructor(height=8, width=8) {
        this.height = height;
        this.width = width;
        this.moves_made = new ArraySet();
        this.mines = new ArraySet()
    ...
    add_knowledge(cell, count) {
        ...
        for (i = x - 1, ii = x + 2; i < ii; i++) {
            for (j = y - 1, jj = y + 2; j < jj; j++) {
                if (i == x && j == y)
                    continue  
                if (0 <= i && i < this.height && 0 <= j && j < this.width) {
                    if (this.mines.has([i, j])) { count -= 1 }
        ...
        for (i = 0; i < n; i++) {
            let s = this.knowledge[i]
            if (s instanceof Sentence)
                if (s.cells.size == 0) this.knowledge.splice(i, i+1); 
            else
                this.knowledge.splice(i, i+1); 
        }
        for (i = 0; i < n; i++) {
            for (j = i + 1; j < n; j++) {
                let s1 = this.knowledge[i]
                let s2 = this.knowledge[j]

                if (isSubSet(s1.cells, s2.cells)) {
                    cells = minusSet(s2.cells, s1.cells)
                    if (cells.size > 0) {
                        var s = new Sentence(cells, s2.count - s1.count)
                        if (!sentenceInArr(s, this.knowledge)) {
                            this.knowledge.push(s)
                        }
                    }                  
                } else if (isSubSet(s2.cells, s1.cells)) {
                    cells = minusSet(s1.cells, s2.cells)
                    if (cells.size > 0) {
                        var s = new Sentence(cells, s1.count - s2.count)
                        if (!sentenceInArr(s, this.knowledge)) {
                            this.knowledge.push(s)
                        }
                    } 
    ...
    make_safe_move() {
        let l = this.safes.size
        const s_i = this.safes[Symbol.iterator]()
        for (i = 0; i < l; i++) {
            let curr = s_i.next().value
